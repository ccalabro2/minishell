...
/*   Updated: 2025/02/21 20:22:18 by gd-auria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void redirect_output(t_exec_manager *tools)
{
	...
	if (tools->cmd->output)
	{
		if (strcmp(tools->cmd->output, PIPE_OUT) == 0 && tools->pipe_std_out >= 0)
			out_fd = tools->pipe_std_out;
		else
		{
			if (tools->cmd->flag)
				out_fd = open(tools->cmd->output, O_WRONLY | O_CREAT | O_APPEND, 0644);
			else
				out_fd = open(tools->cmd->output, O_WRONLY | O_CREAT | O_TRUNC, 0644);
			if (out_fd == -1) {
